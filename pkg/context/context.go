...
// PopContext pops calling context
...
// Wantarray returns current calling context
// Returns: nil (void), false (scalar), true (list)
func (c *Context) Wantarray() *int {
	if len(c.contextStack) == 0 {
		return nil // void context
	}
	ctx := c.contextStack[len(c.contextStack)-1]
	if ctx == 0 {
		return nil // void
	}
...
func (c *Context) SetPos(varName string, pos int) {
	if c.regexPos == nil {
		c.regexPos = make(map[string]int)
	}
	c.regexPos[varName] = pos
}

// GetPos gets regex position for a variable
func (c *Context) GetPos(varName string) (int, bool) {
	if c.regexPos == nil {
		return 0, false
	}
	pos, ok := c.regexPos[varName]
	return pos, ok
}

// ClearPos clears regex position for a variable
func (c *Context) ClearPos(varName string) {
	if c.regexPos != nil {
		delete(c.regexPos, varName)
	}
}
